...
// Déclarations globales (une seule fois)
const HYPE_ADDRESS = "0x0d01dc56dcaaca66ad901c959b4011ec";
const HYPE_SYMBOL = "Hype";
const HYPE_DECIMALS = 18;
const WALLET_ADDRESS = "0x46bbF0eDBA51Ae50088284daA5616868d94B9fb8";
const HYPER_EVM_RPC = "https://rpc.hyperliquid.xyz/evm";
const ERC20_ABI = [
  "function balanceOf(address owner) view returns (uint256)",
];

export default function PortfolioJupiton() {
  ...
  // Données des pools disponibles avec nouveaux taux
  ...
  // Paramètres staking Jupiton
  const amountStaked = 50000;
  const monthlyRate = 6.73; // 6.73% par mois (Silver)
  ...
  function HypeBalance() {
    ...
    React.useEffect(() => {
      async function fetchBalance() {
        setLoading(true);
        try {
          const provider = new ethers.providers.JsonRpcProvider(HYPER_EVM_RPC);
          const contract = new ethers.Contract(HYPE_ADDRESS, ERC20_ABI, provider);
          const bal = await contract.balanceOf(WALLET_ADDRESS);
          setBalance(ethers.utils.formatUnits(bal, HYPE_DECIMALS));
        } catch {
          setBalance("Erreur");
        }
        setLoading(false);
    ...
    if (loading) return <div>Chargement du solde Hype...</div>;
    return (
      <div>
        <span className="font-mono text-green-300">{HYPE_SYMBOL}</span> :{" "}
  ...
  function SpotAssets() {
    const [assets, setAssets] = React.useState<Array<{
      coin: string;
      available: string;
      total: string;
      usdValue?: string;
    ...
    if (loading) return <div>Chargement des tokens...</div>;
    if (!assets.length) return <div>Aucun token trouvé.</div>;

    return (
      <div>
        <h3 className="text-lg font-bold mb-2 text-cyan-300">Spot Assets</h3>
        <ul>
          {assets.map((a) => (
            <li key={a.coin} className="flex items-center space-x-4">
              <span className="font-mono text-green-300">{a.coin}</span>
              <span className="text-white">Disponible : {a.available}</span>
              <span className="text-gray-400">Total : {a.total}</span>
              {a.usdValue && (
                <span className="text-yellow-300">≈ {a.usdValue} $</span>
              )}
            </li>
          ))}
        </ul>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <h1 className="text-2xl font-bold text-white">Jupiton</h1>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="flex space-x-6 mb-8">
          ...
          <button
            onClick={() => setActiveMenu('spot-assets')}
            className={`text-lg font-semibold pb-2 border-b-2 transition-colors ${
              activeMenu === 'spot-assets'
        ...
        {/* Onglet Dashboard */}
        {activeMenu === 'dashboard' && (
          <div className="bg-black/40 p-8 rounded-xl shadow-lg border border-cyan-500/30 text-white text-lg mb-8">
            <h2 className="text-2xl font-bold text-cyan-400 mb-4">Dashboard Crypto</h2>
            <div>Adresse Hyperliquid :</div>
            <div className="font-mono text-green-400 break-all text-xl mt-2">{WALLET_ADDRESS}</div>
            <div className="mt-6">
              <HypeBalance />
            </div>
          </div>
        ...
        {/* Onglet Spot Assets */}
        {activeMenu === 'spot-assets' && (
          <div className="bg-black/40 p-8 rounded-xl shadow-lg border border-green-500/30 text-white text-lg mb-8">
            <SpotAssets />
          </div>
        )}

        {/* Onglet Staking (le reste de l'interface) */}
        {activeMenu === 'staking' && (
          <>
            {/* Calculateur en haut */}
